...
Oldlanguage="`grep "language ="  /config/calibre-server/root/calibre/global.py`" 
if [[ ! "$Oldlanguage" =~ "$WEBLANGUAGE" ]];then
...
if [[ ${ALLLANGUAGE[@]} =~ "$WEBLANGUAGE" ]]; then
sed -i 's/'"$Oldlanguage"'/language = u'"'"''"$WEBLANGUAGE"''"'"'/'   /config/calibre-server/root/calibre/global.py
fi
fi
