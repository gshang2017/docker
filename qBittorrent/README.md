...
|名称|版本|说明|
|:-|:-|:-|
|qBittorrent-qBittorrentEE|5.1.3-5.1.3.10|(amd64;arm64v8;arm32v7) 集成Trackers自动更新|
...
| `-p 8989:8989` |web访问端口 [IP:8989](IP:8989);(默认用户名:admin;(旧)默认密码:adminadmin,(新)初始密码查看容器日志);此端口需与容器端口和环境变量保持一致，否则无法访问|
...
| `本地端口3:8989` |web访问端口 [IP:8989](IP:8989);(默认用户名:admin;(旧)默认密码:adminadmin,(新)初始密码查看容器日志);此端口需与容器端口和环境变量保持一致，否则无法访问|
